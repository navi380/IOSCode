...
class NetworkManager {
  ...
  func posts(query: String, completion: @escaping ([PostImagesModel]?, Error?) -> (Void)) {
    var comp = components()
  ...
  func image(post: PostImagesModel, completion: @escaping (Data?, Error?) -> (Void)) {
    let url = URL(string: post.urls.regular)!
    download(imageURL: url, completion: completion)
  }
  
  func profileImage(post: PostImagesModel, completion: @escaping (Data?, Error?) -> (Void)) {
